...
The tar archive contains ordinary files/directories, including `release.json`, `chipper/chipper`, `chipper/stttest.pcm`, `chipper/webroot/`, `chipper/epod/`, `chipper/intent-data/`, and `lib/libvosk.so`. Dereference library symlinks when packaging. The manifest must contain `source_sha` as the full 40-character lowercase commit ID and `target` as `linux/arm64`. The archive SHA-256 is supplied independently on deployment. Other build metadata may be included in the manifest.
